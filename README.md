...
> [!WARNING]
> **Early alpha (v0.1) — open testing. Use entirely at your own risk.**
> This is a raw, unfinished proof of concept published to share the *idea and
> architecture* and to invite open testing — **not** a finished product. The code
> is immature, the security is **unaudited**, and breaking changes land without
> notice.
>
> - **Do NOT install the server on production or primary servers.** Use throwaway
>   test hosts / VMs you can wipe.
> - **Do NOT run the Windows client on your primary/daily machine.** It installs a
>   TUN adapter, changes routing and DNS, and requires Administrator. Use a spare
>   or virtual machine.
> - No warranty of any kind. You assume **all** risk — data loss, connectivity
>   loss, misconfiguration, or any other damage is entirely your responsibility.
...
>
> The author provides it as-is and takes no responsibility for how it is used or
> for any consequences of running it. If that's not acceptable to you, don't run
> it. What you take away from the design is entirely up to you. 🙂
...
> [!WARNING]
> **Ранняя альфа (v0.1) — открытое тестирование. Всё на ваш страх и риск.**
> Это сырой, незаконченный proof of concept, опубликованный чтобы поделиться
> *идеей и архитектурой* и пригласить к открытому тестированию — **не** готовый
> продукт. Код незрелый, безопасность **не проходила аудита**, ломающие изменения
> прилетают без предупреждения.
>
> - **НЕ ставьте сервер на боевые или основные серверы.** Только на одноразовые
>   тестовые хосты / VM, которые не жалко снести.
> - **НЕ запускайте Windows-клиент на основной/рабочей машине.** Он ставит
>   TUN-адаптер, меняет маршрутизацию и DNS, требует прав администратора.
>   Используйте запасную или виртуальную машину.
> - Никаких гарантий. Вы принимаете **весь** риск — потеря данных, потеря связи,
>   поломка конфигурации или любой другой ущерб целиком на вашей ответственности.
...
>
> Автор предоставляет ПО «как есть» и не несёт ответственности за то, как оно
> используется, и за любые последствия его запуска. Если это вас не устраивает —
> не запускайте. Что вы вынесете из этого дизайна — целиком на ваше усмотрение. 🙂
