...
| `client-windows/` | легаси WebView2 GUI-клиент + общий движок `vpncore` |
...
Server/CLI таргеты кросс-компилируются с любой ОС (`CGO_ENABLED=0`). Легаси
Windows-клиенту нужен `wintun.dll` (с <https://www.wintun.net/>) рядом с исполняемым
файлом.

Flutter-клиент собирается через Flutter SDK (`flutter build windows`); Go-ядро
пробрасывается в Android через gomobile (`gomobile bind ./mobile`).
